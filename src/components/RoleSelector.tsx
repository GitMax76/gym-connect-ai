  ...
  return (
    <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
      {roles.map((role) => (
        <Card
          key={role.id}
          className={`group cursor-pointer transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 border-0 shadow-lg overflow-hidden ${selectedRole === role.id ? `ring-4 ${role.activeRing} shadow-2xl scale-105` : ''
            }`}
        ...
        >
          ...
          <CardContent className="p-8 text-center relative">
            ...
            {/* Content */}
            <h3 className={`text-2xl font-bold text-slate-900 mb-2 transition-colors ${role.hoverText}`}>
              {role.title}
            </h3>
            <p className={`${role.subTitleColor} font-semibold mb-4 text-lg`}>
              {role.subtitle}
            </p>
            ...
            <div className="space-y-2">
              {role.benefits.map((benefit, index) => (
                <div key={index} className="flex items-center justify-center text-sm text-slate-700">
                  <div className={`w-2 h-2 ${role.bulletColor} rounded-full mr-2`}></div>
                  {benefit}
                </div>
              ))}
            </div>
            ...
            {selectedRole === role.id && (
              <div className={`absolute -top-2 -right-2 w-8 h-8 ${role.indicatorColor} rounded-full flex items-center justify-center`}>
